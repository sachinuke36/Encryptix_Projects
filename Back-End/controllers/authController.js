...
const getResume = async (req,res)=>{
  const {token} = req.headers;
  const userId = await jwt.verify(token, process.env.JWT_KEY).id;
  const {name,email,resume} = await jobSeekersCollection.findOne({_id: new ObjectId(userId)});
  if(!name){
   return res.json({success:false, message:'user not found'});
  }
  return res.json({success:true, data:resume, name, email}) 
  
}

const getJobSeekerInfo = async (req,res)=>{
  const {id} = req.params;
  const idArray = id.split(',').filter(id => id.trim() !== '').map(id => new ObjectId(id.trim())); 
  // console.log(idArray)
  try {
    const users = await jobSeekersCollection.find({ _id: { $in: idArray } }).toArray();
    const userInfo = await users.map((i)=>({name:i.name, id:i._id, email:i.email, resume:i.resume}))
...
module.exports = { login, getJobSeekerInfo, getResume, register, validate, loginJobSeekers, registerJobSeeker, updateResume};
